...
class Logger {
  ...
  evaluateLogFileRotation(transport) {
    if (!transport.isStreamReady) transport.currentFileSize = fs.existsSync(transport.filePath) ? fs.statSync(transport.filePath).size : 0;
    const maxFileSize = transport.maxSize * 1024;
    const currentStreamSize = transport._stream ? transport._stream.bytesWritten : 0;
    const requireRotation = transport.currentFileSize + currentStreamSize >= maxFileSize;
    console.log(` -- ${transport.currentFileSize}, ${currentStreamSize}`);

    // do file rotation if applicable
    if (requireRotation) {
      logRotation.sequentialRotation(transport.filePath, 3);
      transport.currentFileSize = 0;
    } else {
      transport.currentFileSize += currentStreamSize;
    }
