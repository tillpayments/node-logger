...
class Logger {
  ...
  convertContext(context) {
    ...
    // if context is an array, return directly
    if (context.constructor === Array) return JSON.stringify(context);

    // handle anything but not object
    if (context.constructor !== Object) return context.toString();
  ...
  buildLogString(level, message, dataDefinition) {
    const category = dataDefinition && dataDefinition.category ? dataDefinition.category : this.category;
    const timestamp = this.getCurrentTimestamp();
    const context = dataDefinition ? dataDefinition.context : null;
    const contextString = context ? `, ${this.convertContext(context)}` : '';
    const data = dataDefinition ? dataDefinition.data : null;
    const dataString = data ? `, ${this.convertContext(data)}` : '';
    return `${timestamp} [${level.toUpperCase()}] (${category})\t${message}${contextString}${dataString}`;
  }

  execLog(level, message, dataDefinition) {
    const levelIndex = LOG_LEVELS.indexOf(level);
    if (levelIndex === -1) level = 'all';
    const logString = this.buildLogString(level, message, dataDefinition);
            ...
            if (transport._stream) {
              ...
              if (transport.writeJson) {
                transport._stream.write(JSON.stringify({
                  ...
                  level,
                  category: dataDefinition.category || this.category,
                  message,
  ...
  getChildLogger(category, context) {
    const childLogger = {
      log: (level, message, data) => {
        this.execLog(level, message, { category: category, context, data});
      }
    };
